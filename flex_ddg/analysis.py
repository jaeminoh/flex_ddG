...
def gam_function(x, score_term=None):
    return -1.0 * np.exp(zemu_gam_params[score_term][0]) + 2.0 * np.exp(
        zemu_gam_params[score_term][0]
    ) / (1.0 + np.exp(-1.0 * x * np.exp(zemu_gam_params[score_term][1])))
...
def apply_zemu_gam(scores):
    ...
    for score_term in zemu_gam_params:
        ...
        scores[score_term] = scores[score_term].apply(
            gam_function, score_term=score_term
        )
...
class Analyzer:
    ...
    def rosetta_output_succeeded(self, potential_struct_dir):
        path_to_rosetta_output = os.path.join(
            potential_struct_dir, self.config.rosetta_output_file_name
        )
        if not os.path.isfile(path_to_rosetta_output):
            return False

        db3_file = os.path.join(potential_struct_dir, self.config.output_database_name)
        ...
        with open(path_to_rosetta_output, "r") as f:
            for line in f:
                if (
                    line.startswith("protocols.jd2.JobDistributor")
                    and "reported success in" in line
                ):
                    success_line_found = True
                if (
                    line.startswith("protocols.jd2.JobDistributor")
                    and "no more batches to process" in line
                ):
    ...
    def find_finished_jobs(self, output_folder):
        return_dict = {}
        job_dirs = [
            os.path.abspath(os.path.join(output_folder, d))
            for d in os.listdir(output_folder)
            if os.path.isdir(os.path.join(output_folder, d))
        ]
        for job_dir in job_dirs:
            completed_struct_dirs = []
            for potential_struct_dir in sorted(
                [
                    os.path.abspath(os.path.join(job_dir, d))
                    for d in os.listdir(job_dir)
                    if os.path.isdir(os.path.join(job_dir, d))
                ]
            ):
                if self.rosetta_output_succeeded(potential_struct_dir):
    ...
    def get_scores_from_db3_file(self, db3_file, struct_number, case_name):
        ...
        def renumber_struct_id(struct_id):
            return self.config.trajectory_stride * (1 + (int(struct_id - 1) // num_batches))
    ...
    def process_finished_struct(self, output_path, case_name):
        db3_file = os.path.join(output_path, self.config.output_database_name)
        assert os.path.isfile(db3_file)
        struct_number = int(os.path.basename(output_path))
        scores_df = self.get_scores_from_db3_file(db3_file, struct_number, case_name)
        ...
        for nstructs in nstructs_to_analyze:
            ...
            ddg_scores = pd.concat(
                [
                    ...
                    scores.loc[
                        (
                            (scores["state"] == "unbound_wt")
                            | (scores["state"] == "bound_mut")
                        )
                        & (scores["struct_num"] <= nstructs)
                    ].copy(),
                ]
            ...
            ddg_scores = (
                ddg_scores.groupby(["case_name", "backrub_steps", "score_function_name"])
    ...
    def calc_dgs(scores):
        ...
        for state in ["mut", "wt"]:
            for nstructs in nstructs_to_analyze:
                ...
                dg_scores = (
                    dg_scores.groupby(["case_name", "backrub_steps", "score_function_name"])
                    .mean(numeric_only=True)
                    .round(decimals=5)
                    .reset_index()
                )
                ...
                scores_list.append(dg_scores)
        return scores_list


    def analyze_output_folder(self, output_folder):
        ...
        finished_jobs = self.find_finished_jobs(output_folder)
        ...
        for finished_job, finished_structs in finished_jobs.items():
            ...
            for finished_struct in finished_structs:
                inner_scores = self.process_finished_struct(
                    finished_struct, os.path.basename(finished_job)
                )
                inner_scores_list.append(inner_scores)
            scores = pd.concat(inner_scores_list)
            ddg_scores, struct_scores = self.calc_ddg(scores)
            struct_scores_dfs.append(struct_scores)
            ddg_scores_dfs.append(ddg_scores)
            ddg_scores_dfs.append(apply_zemu_gam(ddg_scores))
            ddg_scores_dfs.extend(self.calc_dgs(scores))

        if not os.path.isdir(self.config.output_path):
            os.makedirs(self.config.output_path)
        basename = os.path.basename(output_folder)

        pd.concat(struct_scores_dfs).to_csv(
            os.path.join(self.config.output_path, basename + "-struct_scores_results.csv")
        )

        df = pd.concat(ddg_scores_dfs)
        df.to_csv(os.path.join(self.config.output_path, basename + "-results.csv"))
    ...
    def analyze_results(self, pdb_id: str):
        self.analyze_output_folder(os.path.join(self.config.output_path, pdb_id))
